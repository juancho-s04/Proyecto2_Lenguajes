    ...
    <nav class="navbar navbar-expand-lg bg-white border-bottom sticky-top">
        <div class="container">
            ...
            <div class="d-flex gap-2">
                <a class="btn btn-outline-primary" href="{{ url('/vista/usuarios') }}">Volver a usuarios</a>
                ...
                <div class="col-lg-8">
                    <div class="card">
                        <div class="card-body p-4 p-lg-5">
                            ...
                            <h1 class="h2 mb-3 page-title">{{ $formTitle ?? 'Crear usuario' }}</h1>

                            <p class="text-secondary mb-4">
                                {{ $isEdit ? 'Actualiza los datos, rol o contrasena del usuario.' : 'Registra una cuenta para administrador o cliente.' }}
                            </p>
                            ...
                            <form action="{{ $formAction }}" method="POST" class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input id="nombre" name="nombre"
                                        value="{{ old('nombre', $usuarioForm->nombre ?? '') }}" class="form-control"
                                        required>
                                </div>
                                <div class="col-md-6">
                                    <label for="email" class="form-label fw-semibold">Correo electronico</label>
                                    <input id="email" name="email" type="email"
                                        value="{{ old('email', $usuarioForm->email ?? '') }}" class="form-control"
                                        required>
                                </div>
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select id="rolId" name="rolId" class="form-select" required>
                                        ...
                                        @foreach ($roles as $rol)
                                            <option value="{{ $rol->id }}"
                                                {{ old('rolId', $usuarioForm->rolId ?? '') == $rol->id ? 'selected' : '' }}>
                                                {{ $rol->nombre }}
                                            </option>
                                        @endforeach
                                    </select>
